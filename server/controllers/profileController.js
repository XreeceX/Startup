...
const links = await Link.find({ user: user._id, isActive: true }).select("title slug").sort({ createdAt: -1 });
const base = ${req.protocol}://${req.get("host")};
const items = links.map(l => ({ title: l.title || l.slug, url: ${base}/r/${l.slug} }));
res.json({
username,
